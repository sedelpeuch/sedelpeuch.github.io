...
### Tests locaux : utiliser `/etc/hosts`

Pour tester un nom de domaine localement (par exemple faire pointer `example.com` vers une IP de test ou `127.0.0.1`), modifier le fichier local des résolutions permet d'éviter de toucher au DNS public.

- Linux / macOS (commande d'exemple, nécessite les droits administrateur) :

```bash
sudo sh -c "echo '1.2.3.4 example.com' >> /etc/hosts"
```

Pour supprimer l'entrée, éditer `/etc/hosts` et supprimer la ligne ajoutée. Sur macOS, vider le cache DNS si nécessaire :
...
```powershell
Add-Content -Path "$env:SystemRoot\System32\drivers\etc\hosts" -Value "1.2.3.4 example.com"
ipconfig /flushdns
```

Précautions :

- La modification est locale et n'affecte pas le DNS public. Elle nécessite des privilèges administrateur.
- Pour HTTPS, le certificat doit correspondre au nom testé ; sinon le navigateur affichera un avertissement. Utiliser des certificats valides ou accepter le comportement temporairement pour les tests.
- Revenir à la configuration initiale après les tests pour éviter des résolutions imprévues sur la machine de développement.


## Application / Projet lié

### [Cluster Kubernetes SONU](/docs/projects/professionnel/sonu-k8s-cluster)
**Utilisation** : Nginx comme reverse proxy pour exposer les services internes (Grafana, Portainer, SONU Website) et équilibrage de charge.
