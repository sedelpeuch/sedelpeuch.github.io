...
Un workflow réutilisable convient pour encapsuler un pipeline complet (build + test + push) qui s'exécute dans son propre environnement. Une action composite convient pour factoriser une séquence de steps qui s'exécute dans le contexte du job appelant. L'article [architecture CI/CD réutilisable](./2024-12-20-github-actions-architecture-reutilisable.md) combine les deux mécanismes à l'échelle de plusieurs dépôts.
