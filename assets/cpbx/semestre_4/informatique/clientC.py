...
class Client(ConnectionListener):
    ...
    def Network_accepterPseudo(self,data):
        ...
        showinfo("Connexion établie", "Tu es désormais en ligne ! \n Ton pseudo est : {0}".format(self.nickname))
    ...
    def Network_requetePartie(self, data):
        if askyesno("Requête de partie", "Quelqu'un veut jouer avec toi ! \n {0} te propose un REVERSO, acceptes tu ? \n (Il a {1} Points, tu en as (jamais assez) {2})".format(data["pseudo"],data["pointsB"],data["pointsA"])) == True:
    ...
    def Network_coup(self, data): #Permet au joueur d'envoyer au serveur l'information sur son coup.
        global tjeu
    ...
    def demandePartie(self, pseudoJoueur2): #Empêcher de jouer avec soi même 
            if self.nickname ==  pseudoJoueur2:
                    showwarning("Le message s'est perdu en chemin !", "Euh... On a pas programmé un jeu en solo nous !")
                    return None
    ...
    def Network_turnToYou(self,data): #Permet de signaler à l'adversaire que le joueur passe
        mess=self.fenetre.mess
...
Valider = Button(demandeF, text="Valider",bg="gray25",font=police,fg="gray90",command = lambda : demandePseudoServeur(Nickname))
...
def demandePseudoServeur(Nickname): #Permet de demander le pseudo d'un joueur et de le refuser si problème
    if Nickname.get() == "0" or Nickname.get() == "":
        showwarning("Pseudo invalide", "Tu ne peux pas prendre celui là !!")
        Nickname.set("")
    elif len(Nickname.get()) > 16:
        showwarning("Pseudo invalide", "Désolé, les pseudos ne peuvent pas dépasser \n 16 caratères.")
        Nickname.set("")
    else:
...
class Fenetre(): 
    ...
    def initJeu(self,e): #Permet de mettre la phase 1 du jeu en place (mettre les 4 premiers jetons sur le plateau, définir tjeu (ie une double liste permettant
        ...
        for lg in range(1,7):
            ...
            for cl in range(1,7):
                ...
                can.create_oval(x+5,y+5,x+53,y+53, fill = "grey",
                                        outline="grey")
                x += 60
            y += 60
        self.traceGrille()
    ...
    def actualiserJoueurs(self, liste):
        ...
        #
        canvasDefil = Canvas(canvas,width = 480, height = 800, bg ="gray25", scrollregion=(0, 0, 1000, 1000))
        canvasDefil.config(bd=0, highlightcolor = "gray25" , bg = "gray25",borderwidth=0, highlightthickness=0)
        canvasDefil.update()
        #
        ...
        if self.client.occupation != 0:
            for i in range(l):
                    Label(frameDefil, text=joueurs[i], bd=0, width=16,font = fonte, bg="gray25",fg="white").grid(row=l-i, pady=2, column=1,sticky= "ew")
                    Label(frameDefil, text=points[i], bd=0, width=16,font = fonte, bg="gray25",fg="white").grid(row=l-i, pady=2, column=2)
                    Button(frameDefil, text="Inviter" ,width=10,bg='white',fg="gray",command = lambda i=i: showwarning("Pseudo invalide", "Désolé, mais une partie à la fois.")).grid(row=l-i, padx = 2, pady=3, column=0)
            ...
            for i in range(l):
                    ...
                    Label(frameDefil, text=points[i], bd=0, width=16,font = fonte, bg="gray25",fg="white").grid(row=l-i, pady=2, column=2)
                    if etat[i] == 0:
                            Button(frameDefil, text="Inviter" ,width=10,bg='white',fg="green", command= lambda i=i: self.client.demandePartie(joueurs[i])).grid(row=l-i, padx = 2, pady=3, column=0)
                    else:
                            Button(frameDefil, text="Inviter" ,width=10,bg='white',fg="red",command = lambda i=i: showwarning("Pseudo invalide", "Désolé, mais {0} est occupé(e).".format(joueurs[i]))).grid(row=l-i, padx = 2, pady=3, column=0)
