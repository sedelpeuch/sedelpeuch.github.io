...
class FailEndPointTestCase(unittest.TestCase):
    ...
    def runTest(self):
        ...
        self.assertEqual(
            str(self.endpoint_bad.result),
            str(want),
            "Socket got %s instead of %s" % (str(self.endpoint_bad.result), str(want)),
        )
...
class EndPointTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.server = TestServer(
            channelClass=ServerChannel, localaddr=("127.0.0.1", 31426)
        )
    ...
    def runTest(self):
        ...
        for x in range(50):
            ...
            for r in self.endpoint.received:
                self.assertTrue(
                    r["data"] == "Yeah, we got it: %d elements" % self.lengths.pop(0)
                )
        ...
        self.assertTrue(
            self.server.count == self.count,
            "Didn't receive the right number of messages",
        )
        self.assertTrue(
            self.endpoint.count == self.count,
            "Didn't receive the right number of messages",
        )
...
class ServerTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.server = TestServer(
            channelClass=ServerChannel, localaddr=("127.0.0.1", 31427)
        )
