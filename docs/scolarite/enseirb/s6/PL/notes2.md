---
title: PL - Cours 2
description: "Algorithme du simplexe : forme standard, dictionnaires, choix des variables entrante et sortante, pivotage, phase 1, dégénérescence et solutions multiples."
---
...
## Résumé : forme standard, points extrêmes et bases

Les variables supplémentaires $s_i$ sont appelées **variables d'écart**. Chaque
...
Une solution de base réalisable correspond à un point extrême du polyèdre des
contraintes. Pour calculer les valeurs des variables pour ces points on utilise le
...
![Région réalisable de l'exemple](./img/plexemple.png)

Choix de la base initiale : annuler les variables de décision et ne garder que
...
mais jusqu'où augmenter $x$ ? On sait que $s_1, s_2, s_3 \geq 0$ ($y$ restant nul). En posant
...
On remplace $s_3$ par $y$ en base et on réécrit le PL sous forme canonique :
...
Jusqu'où augmenter $s_2$ ? On sait que $s_1,x,y \geq 0$. En posant $s_2 =
...
À chaque itération, la solution courante est associée à un système d'équations
qui la définit :
...
+ La base initiale est formée des variables d'écart $x_{n+1},\ldots,x_{n+m}$,
  c'est-à-dire $\mathcal{B}=\{n+1,\ldots,n+m\}$
...
+ À chaque itération de l'algorithme du simplexe, une variable hors-base va
...
réduits sont négatifs ou nuls : $\overline{c}_j \leq 0$ (la condition est suffisante ; elle est aussi nécessaire en l'absence de dégénérescence).

On choisit une variable $x_k$ de $\mathcal{N}$ dont le coût réduit
$\overline{c}_k$ est strictement positif. Il peut y avoir plusieurs variables candidates
...
indique l'impact sur les variables en base de l'augmentation de la variable
$x_k$.

+ Si $\overline{a}_{i,k} \leq 0$, augmenter $x_k$ ne fait pas diminuer $x_i$ (augmentation ou stagnation) ;
+ si $\overline{a}_{i,k} > 0$, augmenter $x_k$ entraîne une diminution de
  $x_i$ : il faut faire attention à la positivité de $x_i$ :

  $x_i = \overline{b}_i - \overline{a}_{i,k} x_k \geq 0 \Rightarrow x_k \leq \dfrac{\overline{b}_i}{\overline{a}_{i,k}}$

La variable qui sort de la base est la première variable à s'annuler lorsque
$x_k$ (la variable entrante) augmente :

$$
\text{Choisir} \; s \in \mathcal{B} \; \text{tel que} \; s
= \operatorname{argmin}_{i \in \mathcal{B}} \left\{\dfrac{\overline{b}_i}{\overline{a}_{i,k}} :
\overline{a}_{i,k} > 0\right\}
$$

Il peut y avoir plusieurs variables candidates pour sortir de la base. Si c'est
le cas, la base suivante sera dégénérée.
...
+ on élimine $x_k$ de l'expression de $z$ ;
+ on élimine $x_k$ de l'expression des $x_i$ $(i \in \mathcal{B}, i \neq
  s)$.

Il reste à reconstruire le système d'équations ci-dessous pour la nouvelle base :
c'est le pivotage.

$$
\begin{align*} z&=\overline{z}+\sum \limits_{j \in \mathcal{N}\backslash\{k\}}
\overline{c}_j x_j + \overline{c}_k x_k\\
...
On entre la variable $x_k$ en base et on sort $x_s$. On divise l'équation de $x_s$ par $\overline{a}_{s,k}$ pour ramener le
coefficient de $x_k$ à 1. On élimine $x_k$ de l'expression de $z$. On
élimine $x_k$ de l'expression des $x_i$.

### Remarques

+ On remarque que le type de la variable (décision ou écart) n'est pas pris en compte lors du pivotage
...
1. Pour chaque contrainte $i$ telle que $b_i<0$, on ajoute une variable artificielle $x'_{n+i}$ avec un coefficient de -1 en plus de la variable d'écart $\sum_j a_{ij} x_j + x_{n+i} - x'_{n+i} = b_i$
...
4. On résout le problème avec l'algorithme du simplexe à partir de cette solution

### Résultat de la phase 1

À la fin de cette résolution :

+ s'il existe au moins un $i$ tel que $x'_{n+i} > 0$, alors les variables artificielles sont nécessaires pour avoir une solution réalisable : le problème initial est irréalisable ;
+ si $x'_{n+i}=0$ pour tout $i$ tel que $b_i<0$, les variables artificielles sont nulles (si certaines restent en base à la valeur 0, on les fait sortir par pivotage). On a une solution de base réalisable pour le problème initial donnée par la base optimale de la Phase I

1. Supprimer les variables artificielles
2. Reprendre l'objectif initial
3. Utiliser l'algorithme du simplexe avec la solution initiale trouvée
...
Il peut arriver que, dans le dictionnaire optimal, des variables hors base possèdent des coûts réduits nuls : $\overline{c}_i = 0, \; \text{pour} \; i \in \mathcal{N}$. En effectuant une itération supplémentaire du simplexe et en faisant entrer en base une variable $x_i$ telle que $\overline{c}_i=0$, on obtient une nouvelle solution optimale de base (avec le même objectif). Si $x_1^\ast$ et $x_2^\ast$ sont deux solutions optimales, alors toute solution obtenue par une combinaison convexe de $x_1^\ast$ et $x_2^\ast$, $x=\alpha x_1^\ast + (1-\alpha)x_2^\ast$ avec $\alpha \in [0,1]$, est également une solution optimale.
...
+ règle du plus petit indice (règle de Bland), qui garantit la terminaison ;
