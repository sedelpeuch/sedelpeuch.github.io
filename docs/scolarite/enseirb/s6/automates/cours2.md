---
title: Automates finis et langages - Expressions régulières et théorème de Kleene
description: "Langages réguliers, expressions régulières et théorème de Kleene : équivalence entre automates finis, langages réguliers et expressions régulières."
---

Grâce au chapitre précédent, nous pouvons représenter $x$ par un mot et
...
Nous introduisons tout d'abord les opérateurs ensemblistes nécessaires à la
définition des langages réguliers. L'union, l'intersection et la complémentation
sont bien évidemment définies comme pour tout autre ensemble : pour tous langages
$L_1$ et $L_2$,
...
La `fermeture de Kleene` d'un langage $L$ est le langage $L^\ast=\bigcup
\limits_{i \geq 0} L^i$ avec $\begin{cases} L^0 = \{\varepsilon\} \\ L^n
= L \cdot L^{n-1} \end{cases}$. Elle définit le langage des mots qui sont des
concaténations arbitrairement longues (i.e. non bornées) de mots de $L$.
...
3. si $L_1$ et $L_2$ sont des langages réguliers, alors $L_1 \cup
   L_2$, $L_1 \cdot L_2$ et $L_1^\ast$ sont des langages réguliers.
...
verrons par la suite que ces notations décrivent les mêmes objets, mais dans
deux algèbres différentes : respectivement les expressions régulières et les
langages réguliers.

Le `langage d'une expression régulière` est défini par :
...
5. $\mathcal{L}(\alpha^\star)=(\mathcal{L}(\alpha))^\ast$.
...
+ la concaténation est **distributive** à gauche et à droite sur la somme ;
+ l'expression $\varnothing$ est l'élément neutre pour la somme,
  $\varnothing + \alpha = \alpha = \alpha + \varnothing$, et l'élément
...
+ enfin, l'étoile de Kleene est prioritaire sur la concaténation, qui est elle-même
  prioritaire sur la somme.

## Théorème de Kleene

Nous montrons maintenant un lien fondamental entre les automates finis, les
langages réguliers et les expressions régulières.

`Théorème` Les trois propositions suivantes sont équivalentes pour tout langage
$L$ :
...
Le théorème de Kleene est fondamental car il fait le lien entre l'algèbre (i.e.
...
`Lemme` Tout langage représenté par une expression régulière est un langage
régulier.

Les preuves sont disponibles dans le polycopié de cours, page 22.
...
est disponible dans le polycopié de cours, pages 22 à 25.
