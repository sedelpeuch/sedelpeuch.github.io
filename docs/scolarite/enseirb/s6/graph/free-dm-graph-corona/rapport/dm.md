...
description: "Devoir maison de théorie des graphes : simulation de la propagation du Covid-19 sur des graphes circulaires, aléatoires et mixtes, stratégies de tests et de confinement, modèle SIR et recherche de chemin A*."
---
...
décompose en 3 parties d'implémentation : tout d'abord, nous mettons en place les paramètres (mortalité, durée de maladie) et les différents graphes. Vient ensuite l'ajout de
...
derniers. Nous reprenons précisément les définitions du sujet.

### Graphe circulaire

Dans un premier temps, nous allons définir la séquence de graphes $G_i$
permettant de représenter l'état du graphe $G$ au jour $i$. Peu importe le
...
tous les individus, ainsi, peu importe la topologie du graphe nous parcourons le
...
le nombre de jours depuis lequel il est malade s'il est malade. Finalement dans
...
chaque fois nous mettons un 1 dans les cases $(i-1) \bmod n$ et $(i+1) \bmod n$. Dans le
même temps nous remplissons le tableau des voisins de chaque individu. La
création de ce graphe est donc réalisée en `O(n)`, où $n$ est
le nombre d'individus. Nous devons ensuite appliquer le modèle en fonction
de $k$ ou $k'$, cette fonction a une complexité `O(n*k')` : en effet, il faut
faire choisir à chaque individu $k'$ contacts parmi ses $k$
fréquentations. Dans le graphe circulaire $k$ vaut toujours 2, ainsi $k'$
peut valoir 2 et donc cela ne change rien ou 1. À ce stade, nous avons initialisé
...
$i+1$ nous avons une fonction de complexité `O(n*m)`, où $m$ est le nombre
moyen de voisins d'un individu.

Nous pouvons à partir de cela établir notre code final pour le graphe circulaire en modèle
statique et dynamique. Nous initialisons dans un premier temps le graphe et la
...
certaines fonctionnalités nous permettant de récupérer chaque jour une image du
graphe et le nombre de malades, sains, remis ou décédés. *In fine*, notre
algorithme principal a une complexité de `O(nbr jours * n * m)`.

Nous pouvons commencer, comme le sujet nous y invite, à simuler la propagation
dans un graphe circulaire, tout d'abord dans le modèle statique et dynamique
...
![1](./img/circ_50_2_2_14_0.01_0.02.gif)
![2](./img/circ_10000_2_2_14_0.01_0.02.png)

Nous pouvons tout d'abord observer la propagation de la maladie sur le graphe circulaire, cela est conforme à nos attentes, la maladie se propage de voisin en voisin et au bout de 14 jours chaque malade devient soit guéri soit mort. Étant donné que la probabilité de mourir est de $p = 1\%$, nous n'avons aucun mort, car la population est restreinte à 50 individus.

Nous avons en parallèle tracé l'évolution des quatre groupes (malades, guéris, jamais infectés, décédés) sur une population de 10 000 individus, nous voyons qu'à cause de la faible propagation du virus la maladie ne s'est pas diffusée dans la population. Cependant il apparaît clairement que les résultats sont peu satisfaisants,
...
aléatoirement $k'$ voisins parmi ses $k$ contacts autorisés. Grâce à notre
...
![3](./img/random_100_50_3_14_0.01_0.02_stati.gif)
...
définies telles qu'un individu $i$ soit voisin de $(i-1) \bmod n$
et $(i+1) \bmod n$ et au maximum $k'-2$ voisins choisis aléatoirement parmi
...
possibles. La boucle de simulation est donc en complexité `O(nbr jours * n * m)`.
...
permettant de simuler la propagation d'une maladie dans une population ayant des
interactions ressemblant à des relations humaines. Nous avons actuellement une modélisation d'une propagation naturelle d'une maladie, cette maladie se propage rapidement et toute la population est rapidement touchée. Nous avons dans nos modèles peu de morts mais si nous avions 67 000 000 d'individus cela représente tout de même 670 000 personnes (70 millions de morts sur l'ensemble de la planète sans aucune mesure pour contenir et ralentir la maladie).

## Partie II - Ajout de tests simples (Sébastien & Antoine)
...
particulièrement attentifs à la taille du pic épidémique et au nombre de morts
total.
...
tests dans ce rapport ne peut aller sans l'évocation du paradoxe des faux positifs (négligence de la fréquence de base) : lorsque la maladie est rare, la majorité des tests positifs peut correspondre à des personnes saines, et une lecture naïve des résultats induit donc en erreur. (Ce phénomène est souvent confondu à tort avec le paradoxe de Simpson, qui désigne l'inversion d'une tendance lorsqu'on agrège des sous-groupes.)

![Arbre de probabilités du test](./arbre.png)

Lorsque nous effectuons un test lors d'une épidémie, ce paradoxe est très facile à montrer. Prenons les conditions du sujet, la maladie
...
positive avec $10\%$ de chance. L'arbre ci-dessus résume la situation où $M$
...
positifs et 8910 non malades testés négatifs. Le paradoxe est mis en lumière : le
nombre de malades selon le test est supérieur au nombre de malades réel, et il y a
même plus de gens non malades testés positifs que de malades testés
positifs (seuls $70/1060 \approx 6,6\%$ des tests positifs correspondent à des malades). L'utilisation des tests est donc délicate : ainsi nous
...
partie de la population qui est vraiment malade (dans notre cas $0,3\%$).
...
les personnes l'ayant contactée dans les $r'\leq r$ derniers jours sont placées en
...
voisins fréquentés depuis $r$ jours. Ce tableau est un tableau de tableaux
contenant à chaque ligne les voisins que l'individu a fréquentés. Cela nous
...
donne une complexité à la boucle principale en temps `O(nbr jours * n * m * r)`,
où $r$ est le nombre de jours que dure la maladie et $m$ le nombre moyen de
voisins.
...
gens en contact avec un défunt qui correspond à une réaction spontanée. Il
...
choisissons aléatoirement $n'$ individus à tester (pour coller à la réalité $n'$
...
Nous réalisons alors des tests aléatoires sur la population avec les paramètres de la maladie du sujet. Pour nous donner une première intuition nous réalisons l'animation suivante

![19](./img/propagation_tests_alea_iso-100-10-5-14-0.15-0.05.gif)

Nous avons une première intuition de l'effet des tests aléatoires sur la population, ils permettent de contenir la maladie et éviter qu'elle se propage, nous voyons aussi l'effet des faux positifs, puisque les tests continuent à s'effectuer et à confiner des gens alors que la maladie a disparu. Nous allons montrer sur un plus grand échantillon de la population l'efficacité des tests aléatoires. Nous utilisons le confinement fort puis faible avec un graphe mixte dans le modèle dynamique.

![25](./img/stat-conffaible-test_alea.png)
![26](./img/stat-conffort-test_alea.png)
...
En somme nous avons vu une autre politique de test qui permet de réduire drastiquement le pic épidémique ce qui est un point positif. Nous pouvons estimer que ce genre de test combiné à un confinement fort permet un contrôle efficace de la maladie en comparaison avec les tests sur les proches des défunts.
...
préventifs. Nous allons maintenant imaginer une stratégie de tests où nous
...
Nous avons donc une première intuition, la combinaison des deux politiques de tests a l'air très efficace car la maladie se propage très peu et est vite contenue. Cependant, cette propagation est biaisée puisque la population est très réduite.

Nous allons alors comparer les proportions des différentes populations en
effectuant un test sur 1 000 individus, avec les paramètres de la maladie
précédents, en utilisant tout d'abord le confinement fort puis le faible.
...
connexe, cela contiendrait la maladie à un groupe, cependant la définition du
...
la population de gens jamais infectés est très grande. Le confinement fort
...
avons, malgré les faux positifs, mais c'est aussi la plus coûteuse (du fait du
...
les faux positifs, une propagation très rapide de la maladie qui irait plus vite
...
le premier ne meure). Cela conclut cette partie et l'implémentation de base de
notre modélisation, nous allons maintenant passer à une partie d'ouverture.

## Partie III - Outils et améliorations

Nous allons maintenant discuter d'outils que nous avons ajoutés dans ce projet
...
#### Sébastien (Théorie & Rapport), Antoine (Implémentation dans GeoGebra)

Cette partie présente un autre mode de représentation de la propagation de la
maladie. La représentation se fera par équations différentielles en s'inspirant
du travail de [David
...
s'inspire des pages précédemment mentionnées. L'idée est de présenter une
représentation qui a intéressé les membres du groupe et sur laquelle nous avons
réalisé un travail de recherche, l'implémentation réalisée avec GeoGebra est de
notre œuvre.
...
qui relie les dérivées des fonctions $\dfrac{dS(t)}{dt}$,$\dfrac{dI(t)}{dt}$
et $\dfrac{dR(t)}{dt}$, aux fonctions elles-mêmes $S(t),I(t)$ et $R(t)$.
Les valeurs de $S$, $I$ et $R$ sont toujours positives et sans dimension.
...
l'effectif de la population saine multiplié par un coefficient de propagation
...
pour l'intégrer ainsi $dI(t)/dt=qIS-I/r$. Finalement, nous devons ajouter un terme
représentant la mortalité de la maladie, nous réécrivons alors l'équation en
...
\dfrac{I}{r} - pI \\ \dfrac{dR(t)}{dt} &= \dfrac{I}{r}
\end{align*}
$$

Sans le terme de mortalité ($p = 0$), on retrouve le modèle SIR classique, pour lequel $S+I+R$ reste constant.
...
manipuler les notions d'algorithmique des graphes et l'implémentation de ces notions*
...
distance autorisée le nombre de malades. Pour cela, nous modélisons un monde
...
L'algorithme A* est un algorithme basé sur un principe très simple. À chaque
itération, nous allons tenter de nous rapprocher de la destination, nous allons
...
les solutions mises de côté qui nous garantit que l'algorithme trouvera
toujours une solution si elle existe. De plus, si l'heuristique utilisée est admissible (elle ne surestime jamais la distance restante, comme la distance à vol d'oiseau), le chemin trouvé est un plus court chemin.

Nous pouvons donc lui donner un terrain avec autant d'obstacles que nous
voulons : s'il y a une solution, A* la trouvera. Un résumé du fonctionnement de l'algorithme est disponible sur l'animation ci-dessous.

![Fonctionnement de l'algorithme A*](./astar.gif)
...
La complexité de cet algorithme dépend de l'implémentation des structures de données des listes et du temps d'accès à leurs éléments. Nous utilisons un tas : dans le pire des cas, la complexité de A* est alors celle de Dijkstra, `O((n + m) log(n))`, avec $n$ le nombre de sommets et $m$ le nombre d'arêtes.

Nous avons alors implémenté notre Astar en python, ce dernier se base sur un
A* développé en C++ réalisé par Sébastien dans le cadre de Eirbot. Voici les [sources du Github Eirbot](https://github.com/eirbot/eirbot2020-1A/blob/master/code/rasp/src/navigation.cpp)
...
itérations suivantes ramènent à des résultats que nous avons déjà présentés.

Tout d'abord si les individus n'ont aucune limite sur leurs déplacements. Les
contacts sont représentés par une marque grise et les marques rouges
correspondent à une infection.

![12](img/Astar_deconf.png)

Nous voyons que nous obtenons beaucoup de contacts et donc d'infections, ainsi
...
Nous avons donc réalisé une modélisation simpliste de la propagation du
...
notre modèle. La répartition des tâches est disponible tout au long du rapport, nous l'avons résumée via gitinspector dans l'image suivante
