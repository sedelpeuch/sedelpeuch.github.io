...
description: "Couches session, présentation et application du modèle OSI : gestion du dialogue, synchronisation, syntaxes abstraite et de transfert, ASN.1."
---

Rappel des modèles OSI et TCP/IP :

![Modèles OSI et TCP/IP](./img/1.png)

## Couche session

La couche session est la première couche qui ne s'occupe pas de l'acheminement des données
...
La correspondance entre la couche de session et la couche transport se fait selon
3 cas possibles :

+ Correspondance **biunivoque** : la session et la connexion de transport sont de même durée et superposables

![Correspondance biunivoque](./img/2.png)

+ Plusieurs sessions **successives** sont établies sur une même connexion de transport

![Sessions successives sur une connexion](./img/3.png)

+ Une même session s'étend sur plusieurs connexions de transport (arrêt /
  reprise)

![Session sur plusieurs connexions](./img/4.png)
...
la couche session est l'établissement et la libération de la connexion, avec un
...
dialogue par jeton où seul le détenteur du jeton peut transmettre des données,
...
+ **Encodage** dans une norme agréée permettant à des équipements de communiquer

Le rôle de la couche présentation est **d'assurer le codage** et **décodage**
des données. La représentation des données (chaînes de caractères, nombres
...
données et en faisant précéder le codage d'une donnée par une étiquette qui en
précise la nature.
...
ASN.1 (*Abstract Syntax Notation One*) permet de décrire les types de données qui peuvent être codées et
transmises au travers de la couche présentation (les règles de codage associées, comme BER, définissent la syntaxe de transfert). Exemples d'utilisation :
...
+ MIB (Management information base), ensemble des objets gérés par SNMP, objets
  de la MIB référencés par des identificateurs ASN.1
