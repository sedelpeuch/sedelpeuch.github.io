---
title: "La mémoire"
description: "Gestion de la mémoire centrale : allocation contiguë, registres barrière et de base, swap et algorithmes de placement (first fit, best fit, worst fit)."
sidebar_position: 7
---
...
multitâche, ceci pour maximiser l'utilisation du CPU. Cette technique pose
comme condition obligatoire que la mémoire centrale soit partagée
...
liberté quant à l'utilisation de la mémoire. En effet, toute adresse est
accessible et peut être utilisée pour n'importe quelle tâche. L'inconvénient :
aucune fonctionnalité, tout doit être reprogrammé, typiquement il n'y a pas de
système d'exploitation.

### Le moniteur résident

On cherche à protéger le noyau des interférences possibles de la part des
...
est supérieure à la barrière, l'adresse est légale, sinon l'adresse est une
...
Globalement, ce temps supplémentaire peut être négligé.

### Le registre barrière

L'implémentation d'un tel mécanisme doit être réalisée de façon matérielle. La
valeur du registre barrière est parfois fixée matériellement sur une machine,
ce qui pose des problèmes dès que l'on veut changer le noyau ou protéger plus
de mémoire.
...
qui commencent à 0000. Pour continuer à fournir cette possibilité, le registre
barrière est transformé en registre de base. À chaque utilisation d'une adresse
logique par le programme, on ajoute à cette adresse la valeur du registre de base pour trouver
...
des systèmes à temps partagé, en utilisant le mécanisme de swap. Swapper, c'est
...
général un disque, mais on peut utiliser d'autres supports secondaires plus lents
ou plus rapides, comme des bandes.

### Le coût du swap

Sur un tel système, le temps de commutation de tâches est très important. Il est
donc nécessaire que chaque processus dispose de l'unité de traitement pendant un
temps suffisamment long pour que le ralentissement dû au swap ne soit pas trop
sensible. Que se passe-t-il sinon ? Le système utilise la majeure partie de ses
ressources à déplacer des processus en et hors mémoire centrale. L'unité de
...
augmentations ou réductions de taille d'un processus utilisateur soient réalisées
...
Les choix d'implémentation des mécanismes d'adressage influencent énormément
l'ordonnancement des processus. Nous nous plaçons dans le cas d'un système
multiprogrammé où les processus restent en mémoire pendant tout le temps de leur
exécution. S'il n'y a plus de place le processus
...
mémoire à un processus). On distingue en général trois méthodes :

+ **First fit** : le premier bloc suffisamment grand pour contenir notre processus
  est choisi
+ **Best fit** : le plus petit bloc suffisamment grand pour contenir notre
  processus est choisi
+ **Worst fit** : le plus grand bloc libre est choisi, ce qui laisse le plus
  grand morceau de mémoire libre.

Les simulations montrent que first fit et best fit sont plus efficaces que worst
fit, en temps comme en utilisation de la mémoire. Aucun des deux ne l'emporte
nettement sur l'autre en utilisation de la mémoire, mais first fit est en
général plus rapide.
