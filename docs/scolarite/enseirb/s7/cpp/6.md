...
description: "Notes de cours C++ : espaces de nommage nommés et anonymes, alias, déclarations et directives using."
---
...
morceau sert de déclaration, et les suivants d'extensions. La syntaxe pour une
...
L'accès aux identificateurs des espaces de nommage se fait par défaut grâce à
...
l'intérieur d'une fonction ou à l'intérieur d'une classe. Depuis C++17, la
syntaxe `namespace A::B { ... }` permet de déclarer directement un espace de
nommage imbriqué.

### Espaces de nommage anonymes

Lorsque, lors de la déclaration d'un espace de nommage, aucun nom n'est donné,
un espace de nommage anonyme est créé. Ce type d'espace de nommage permet
...
rendre uniques des identificateurs dans un fichier (depuis C++11, les noms qu'ils
contiennent ont d'ailleurs une liaison interne). Cependant, ils sont plus
puissants, parce que l'on peut également déclarer des espaces de nommage
...
`nom_alias` est ici le nom de l'alias de l'espace de nommage, et `nom` est le nom
...
Les déclarations `using` permettent d'utiliser un identificateur d'un espace de
nommage de manière simplifiée, sans avoir à spécifier son nom complet (c'est-à-dire le nom
...
Si l'espace de nommage concerné par la déclaration `using` est étendu après
...
même nom, soit ces identificateurs doivent tous se rapporter au même objet, soit
ils doivent représenter des fonctions ayant des signatures différentes (les
fonctions déclarées sont alors surchargées). Dans le cas contraire, des ambiguïtés
...
elle est utilisée. De plus, l'identificateur donné à la déclaration `using` doit
être accessible dans la classe de base (c'est-à-dire `protected` ou
`public`).
...
signature est définie dans la classe dérivée, cette dernière fonction masque
la fonction de la classe de base (ou la redéfinit si elle est virtuelle). Il
n'y a pas d'ambiguïté dans ce cas.

## Directive `using`

La directive `using` permet d'utiliser, sans spécification d'espace de nommage,
...
déclarées jusqu'à la fin du bloc de portée courante. Si un espace de nommage est
...
Il se peut que lors de l'introduction des identificateurs d'un espace de
nommage par une directive `using`, des conflits de noms apparaissent. Dans ce
cas, aucune erreur n'est signalée lors de la directive `using`. En revanche, une
erreur se produit si un des identificateurs pour lesquels il y a conflit est
utilisé. C'est pour cette raison qu'il est déconseillé de placer une directive
`using namespace` (par exemple `using namespace std;`) dans un fichier d'en-tête :
elle s'appliquerait à tous les fichiers qui l'incluent.
