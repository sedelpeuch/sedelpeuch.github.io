---
title: "Les templates"
description: "Notes de cours C++ : paramètres template, fonctions et classes template, fonctions membres template, instanciation implicite et explicite."
---
...
fonctions travaillant sur de nombreux types grâce à la surcharge.
...
serait possible d'utiliser des macros pour faire des fonctions non typées, mais
cela serait au détriment de la taille du code et du contrôle des types.

Le C++ permet de résoudre ces problèmes grâce aux paramètres génériques, que
l'on appelle encore paramètres template. Un paramètre template est soit un type
...
qui peuvent être paramétrées par une constante de type intégral. Les classes
...
être remplacé indifféremment dans cette syntaxe par le mot-clé `typename`
(l'inverse n'est pas vrai ailleurs : seul `typename` permet d'indiquer qu'un
nom dépendant d'un paramètre template, comme `typename T::value_type`, désigne
un type). La même
...
paramètre d'un signe égal et de la valeur. Ici, la valeur par défaut doit
...
n'importe quel type du langage déjà déclaré au moment où la déclaration template
...
déclaration `template`. Pour une classe template, la ligne suivante provoquera donc une erreur de
compilation : `template <class T=int, class V>`.

Il est possible d'utiliser une classe template en tant que type générique. Dans
...
clés et les valeurs sont stockées parallèlement dans les membres Clef et Valeur. Ces
...
La déclaration des paramètres constants se fait de la manière suivante :

```text
template <type paramètre[=valeur][,...]>
```

où `type` est le type du paramètre constant, `paramètre` est le nom du paramètre
et `valeur` est sa valeur par défaut. Il est possible de donner des paramètres
...
Ce sont donc tous les types qui peuvent être assimilés à des valeurs entières
(entiers, énumérés ou adresses). Depuis C++20, les types flottants et certains
types classes littéraux sont également autorisés.
...
La déclaration et la définition des fonctions template se font exactement comme
s'il s'agissait de fonctions normales, à ceci près qu'elle doit être précédée de la
...
où `paramètres_template` est la liste des paramètres template et
...
Les paramètres template qui sont utilisés dans la liste des paramètres de la
fonction peuvent être déduits par le compilateur à partir des types des
arguments lors de l'appel. Les autres doivent être précisés explicitement lors
de l'appel (par exemple `f<int>()`) ou avoir une valeur par défaut.
...
des variables locales qualifiées `const`. Les
...
classiques que par d'autres fonctions template. Lorsqu'une fonction template et une fonction normale qui la surcharge
conviennent aussi bien l'une que l'autre pour un appel, la fonction classique
est préférée ; la fonction template n'est retenue que si elle fournit une
...
template de la liste `paramètres_template` dans `paramètres`, séparés par des
...
qu'on voudra la référencer. Cette règle est cependant facultative lorsque la
classe est référencée à l'intérieur de sa propre définition ou d'une de ses
fonctions membres.
...
{
    if (&Init == this) return *this;   // Autoaffectation.
    flush();
    StackItem *tmp1 = Init.Tete, *tmp2 = NULL;
...
fonctions soient elles-mêmes template ou non.
...
syntaxe est exactement la même que pour les fonctions template non membres.
...
template de la classe, et la fonction membre template s'écrit donc exactement
comme une fonction template classique.
...
class D : public B
{
    ...
    void f(int i)  // Cette fonction redéfinit B::f(int).
    {
        f<>(i);    // Elle appelle la fonction template.
        return ;
    }
};
```
...
fournir les valeurs pour tous les paramètres qui n'ont pas de valeur par défaut.
...
une expression qui utilise pour la première fois une fonction ou une classe
...
être déduite de l'expression à compiler. Par exemple, les fonctions
...
```cpp
int i = Min(2, 3);         // T est déduit : int.
int j = Min<int>(2, 3.0);  // T est forcé à int.
```

Dans la première ligne, le type `int` est déduit des arguments. Dans la seconde,
les deux arguments ont des types différents et la déduction échouerait : le
paramètre template est donc forcé à `int`, et `3.0` est converti en entier.

On prendra garde au fait que le compilateur utilise une politique minimaliste
...
exemple, la définition d'un objet d'une classe template dont tous les paramètres
sont définis provoque l'instanciation de cette classe, mais la définition d'un
...
```cpp
template class Stack<int>;             // Instancie toute la classe Stack<int>.
template int Min<int>(int, int);       // Instancie Min<int>.
```

Depuis C++11, la forme `extern template class Stack<int>;` indique à l'inverse
qu'une instanciation explicite existe dans une autre unité de traduction, ce
qui évite de la générer dans chaque fichier.
