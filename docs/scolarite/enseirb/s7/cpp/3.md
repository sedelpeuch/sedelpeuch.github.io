...
description: "Notes de cours C++ : surcharge des opérateurs d'affectation, de transtypage, de comparaison, d'incrémentation, d'appel de fonction, de déréférencement et d'allocation."
---
...
Pour une classe simple (par exemple une classe de nombres complexes),
l'opérateur d'affectation est trivial, mais ce n'est pas toujours le cas, et l'implémentation des opérateurs d'affectation peut parfois
soulever quelques problèmes.

Premièrement, comme nous l'avons dit plus tôt, le fait de définir un opérateur
...
copie, les écritures telles que `classe objet = source;` ne fonctionneront pas
...
Par exemple, il faut s'assurer que l'opérateur d'affectation ne cherche pas à
...
Ainsi, si la copie échoue pour une raison ou une autre, l'objet courant n'est
pas modifié et reste dans un état stable (idiome *copy-and-swap*). Le code
permettant de réaliser ceci est le suivant, en supposant que la classe fournit
une méthode `swap` qui échange les données membres sans lever d'exception :
...
classe &classe::operator=(const classe &source)
{
    ...
    // (std::swap(Temp, *this) appellerait operator= : récursion infinie) :
    swap(Temp);
...
Depuis C++11, une variante consiste à prendre le paramètre par valeur
(`classe &operator=(classe source)`) : la copie (ou le déplacement, si l'argument
est temporaire) est alors réalisée à l'appel, et le corps se limite à l'échange.
...
en chaînes C classiques (c'est-à-dire en tableaux de caractères) si l'opérateur
...
tout l'objet qui l'appelle, mais surtout il n'a pas de type de retour déclaré. En effet, puisque
c'est un opérateur de transtypage, son type est nécessairement celui qui lui
correspond (dans le cas présent, `char const *`). Depuis C++11, un opérateur de
transtypage peut lui aussi être déclaré `explicit`, afin d'interdire les
...
Depuis C++20, l'opérateur de comparaison à trois voies `operator<=>` permet au
compilateur de dériver `<`, `<=`, `>` et `>=`, et un `operator==` déclaré
`= default` compare les membres un à un.
...
revanche renvoient par valeur une copie de l'objet avant modification.
...
L'opérateur fonctionnel est également très utile en raison de son n-arité. Il
...
class matrice
{
    ...
    unsigned short int m;   // Nombre de colonnes (2e paramètre).
...
## Opérateurs de déréférencement et d'adresse

L'opérateur de déréférencement (ou d'indirection) `*` permet l'écriture de
classes dont les objets peuvent être utilisés dans des expressions manipulant
des pointeurs. L'opérateur d'adresse `&`, quant à lui, permet de renvoyer une
adresse autre que celle de l'objet sur lequel il s'applique (sa surcharge est
rarement souhaitable ; `std::addressof` permet d'obtenir la véritable adresse).
Enfin, l'opérateur de déréférencement et de sélection de membres de structures
`->` permet de réaliser des classes qui encapsulent d'autres classes (c'est le
principe des pointeurs intelligents).

Si les opérateurs `*` et `&` peuvent renvoyer
...
La forme la plus simple de new ne prend qu'un paramètre : le nombre d'octets à allouer, qui vaut toujours la taille de l'objet à construire. Il doit renvoyer un pointeur de type `void *`. L'opérateur delete correspondant peut prendre, quant à lui, soit un, soit deux paramètres. Comme on l'a déjà dit, le premier paramètre est toujours un pointeur de type `void *` sur l'objet à détruire. Le deuxième paramètre, s'il existe, est du type `size_t` et contient la taille de l'objet à détruire. Les mêmes règles s'appliquent pour les opérateurs new[] et delete[], utilisés pour les tableaux.
...
où `taille` est la taille de la zone de mémoire à allouer et `paramètres` la
liste des paramètres additionnels, alors on doit l'appeler avec la syntaxe
`new(paramètres) classe;`

Les paramètres sont donc passés entre parenthèses comme pour une fonction normale. Le nom de la fonction est new, et le nom de la classe suit l'expression new comme dans la syntaxe sans paramètres. Cette utilisation de new est appelée new avec placement. La bibliothèque
standard fournit déjà, dans l'en-tête `<new>`, la forme
`void *operator new(std::size_t, void *)`, qui renvoie simplement le pointeur
reçu : `::new (adresse) T(args)` construit un objet à une adresse donnée.
...
int main(void)
{
    A *pA=new A;      // Création d'un objet de classe A.
                      // L'opérateur new normal de A (malloc) est utilisé.
...
lance une exception (voir le cours sur les exceptions). Dans
