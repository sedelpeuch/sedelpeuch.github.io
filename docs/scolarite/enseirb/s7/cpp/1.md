---
title: "La couche objet - Partie 1 : classes, héritage, classes de base virtuelles, fonctions et classes amies"
description: "Notes de cours C++ : déclaration de classes, encapsulation, héritage, classes de base virtuelles, fonctions et classes amies."
---
...
qui leur sont appliquées. En tout cas, les données et le code ne se mélangent pas
...
nouvel objet créé. L'initialisation de l'objet est réalisée par une méthode
spéciale, le constructeur. Lorsque l'objet est détruit, une autre méthode
...
langage C, c'est une véritable extension. En effet, les notions qu'elle a
...
orientée objet. Au sens strict de la norme, les types fondamentaux ne sont
toutefois pas des classes : il est impossible d'en hériter ou de leur ajouter
des méthodes ; l'analogie porte sur leur manipulation au travers des opérateurs.
...
nouveau type se fait donc en définissant la classe des variables de ce type.
...
appartiennent et de `::`. Cet opérateur `::` est appelé l'opérateur de
...
cours. Les champs d'une classe peuvent être accédés comme des variables normales
...
cette classe, elles sont implicitement déclarées `inline` (le compilateur
reste libre de les développer ou non en ligne). Si les méthodes ne sont
...
`clientele[0].Solde = 25000;`. Le solde d'un client peut donc être modifié sans
...
faut faire précéder ceux-ci du mot-clé indiquant les droits d'accès suivi de
...
Les classes construites avec `class` ont tous les membres privés. Il en va de
même pour le mode d'héritage par défaut (`public` pour `struct`, `private` pour
`class`).

## Héritage

L'héritage permet de donner à une classe toutes les caractéristiques d'une ou de
...
classe fille, classe dérivée ou classe descendante.
...
Les mots-clés `private` et `protected` peuvent remplacer `public` dans la liste
d'héritage ; ils restreignent alors l'accès aux membres hérités.
...
l'opérateur de résolution de portée, suivi du nom du membre.
...
(`dynamic_cast`), ce qui suppose que la classe de base soit polymorphe (au moins
une méthode virtuelle). Cet opérateur sera décrit plus tard.
...
classe virtuelle. Pour reprendre l'exemple donné ci-dessus, si les classes B et
C appelaient toutes les deux un constructeur non trivial de la classe virtuelle
A et que la classe D appelait elle-même les constructeurs de B et C, le sous
...
de base virtuelles dans les classes de base intermédiaires : seul le
constructeur de la classe la plus dérivée initialise la base virtuelle. Il faut
donc systématiquement le spécifier, à chaque niveau de la hiérarchie de classes. La
...
d'analyse dans la hiérarchie des classes, mais pas toujours. Elles restent donc
...
puisque les méthodes ont déjà accès aux membres de la classe).
...
sera pas une sous-classe de la classe cible, mais une classe de l'espace de nommage englobant :
