...
description: "Notes de cours C++ : lancement et capture des exceptions, relance, spécifications d'exceptions, hiérarchies d'exceptions et exceptions dans les constructeurs."
---
...
gestionnaire d'exception. Lorsqu'un gestionnaire d'exception s'exécute, on dit qu'il
...
terminer normalement. Comme son traitement n'a pas pu se dérouler normalement,
...
très lourde, puisque ces tests s'imbriquent les uns à la suite des autres et que
...
distinguer les différents cas d'erreur possibles, car il peut en découler un
grand nombre de tests et beaucoup de cas particuliers à gérer dans les fonctions
appelantes.
...
réalisé vers la partie du code de nettoyage correspondant au traitement qui a
...
doivent donc être placées dans une portée relativement globale, voire déclarées en
tête de fonction. De plus, le traitement des codes d'erreurs multiples pose
toujours les mêmes problèmes de complication des tests.

La solution qui met en œuvre les exceptions est beaucoup plus simple, puisque
...
état cohérent, la remontée des exceptions effectue automatiquement le ménage (c'est le
principe RAII, *Resource Acquisition Is Initialization*). De
...
termes de fonctionnalités aux codes d'erreurs utilisés précédemment.
...
de lancer une exception dans un bloc d'instructions particulier. Ce bloc est
...
l'exception est relancée après traitement. La capture par référence (de
préférence constante, `catch (const erreur &e)`) évite en outre la troncature
(*slicing*) d'une exception de classe dérivée capturée par sa classe de base.
...
:::caution Mécanisme supprimé du C++ moderne

Les spécifications dynamiques d'exceptions `throw(liste)` décrites dans cette
section ont été dépréciées en C++11 et supprimées en C++17, de même que
`std::unexpected` et `std::set_unexpected`. La forme vide `throw()` a été
conservée comme synonyme de `noexcept` jusqu'à C++20, qui l'a supprimée à son
tour. Les exemples ci-dessous ne compilent donc qu'avec un standard antérieur à
C++17 (par exemple `-std=c++14`).

Le C++ moderne ne distingue plus que deux cas : une fonction déclarée `noexcept`
s'engage à ne laisser s'échapper aucune exception (si une exception s'en échappe
malgré tout, `std::terminate` est appelée) ; les autres fonctions peuvent lever
n'importe quelle exception. Les destructeurs sont implicitement `noexcept`.
...
suit (en C++98) dans le fichier d'en-tête `<exception>` :
...
Si `std::bad_exception` figure dans la liste des exceptions autorisées,
l'exception non autorisée est remplacée par un objet `std::bad_exception` ;
sinon, `std::terminate` est appelée et le programme se termine.
...
pour cet objet, puisque cela n'a pas de sens (les sous-objets déjà
construits, classes de base et données membres, sont en revanche détruits).
Cependant, ce comportement soulève
...
(*function-try-block*), qui couvre aussi la liste d'initialisation. Les blocs
...
de celui des blocs `catch` classiques. En effet, les exceptions ne sont
...
bloc `try` cependant, l'exception est systématiquement relancée. De plus, lorsque ce bloc `catch` s'exécute, les données membres et les classes
de base ont déjà été détruites : y accéder a un comportement indéfini. Ce bloc
sert donc essentiellement à journaliser l'erreur ou à la traduire en une autre
exception ; la libération des ressources déjà acquises doit se faire avant
(dans un bloc `try` interne au corps du constructeur) ou, mieux, être confiée à
des membres RAII (`std::unique_ptr`, `std::vector`...). Il faut toujours capter
...
try
{
    ...
    {
        // Les membres sont encore accessibles ici.
...
{
    // Les membres sont déjà détruits : on ne doit plus y accéder.
    cout << "Construction de A impossible" << endl;
...
traitée dans le bloc `catch` interne, qui libère le tampon, puis dans le bloc
`catch` qui suit la définition du constructeur de la classe `A`, avant d'être
relancée vers `main`. L'opérateur `delete` est bien appelé automatiquement, mais le destructeur
de `A` n'est jamais exécuté.

En général, si une classe hérite d'une ou plusieurs classes de base, l'appel
