---
title: "La couche objet - Partie 4 : entrées/sorties, méthodes virtuelles, dérivation, classes abstraites"
description: "Notes de cours C++ : flux d'entrée/sortie, méthodes virtuelles, règles de dérivation, destructeurs virtuels, classes abstraites et pointeurs sur membres."
---
...
sortie. Ces classes réalisent en particulier les opérations d'entrée / sortie depuis
et vers les périphériques d'entrée et les périphériques de sortie standards
...
tandis que l'opérateur `>>`, ou opérateur d'extraction, permettra de réaliser la
...
flux d'entrée standard `stdin` du programme, et l'objet `cout` au flux de
...
d'information. Les données écrites sur ces deux flux aboutissent dans le même flux ; la
différence est que `cerr` n'est pas tamponné (chaque écriture est immédiatement
vidée), alors que `clog` l'est. L'emploi de l'objet `clog` est assez rare.
...
sorties successivement sur un même flux.

De plus, la bibliothèque standard définit ce que l'on appelle des manipulateurs
...
saut de ligne lorsqu'il est employé sur un flux de sortie. Il vide également
le tampon du flux, ce qui le rend plus coûteux qu'un simple `'\n'`.
...
utilisé dans un contexte et dans un sens différents.
...
appelée pour un objet de la classe A, soit la fonction y de la classe B si elle
est appelée pour un objet de la classe B. Le lien avec l'une des méthodes y ne
doit être fait qu'au moment de l'exécution, c'est-à-dire qu'on doit faire une
liaison dynamique (ou tardive).

Le C++ permet de faire cela. Pour cela, il suffit de déclarer virtuelle la
fonction de la classe de base qui est redéfinie dans la classe fille, c'est-à-dire
la fonction y. Cela se fait en faisant précéder sa déclaration du mot-clé
...
Si `d` est un objet de la classe `DonneeDetaillee`, l'appel de `d.Entre` ne
causera pas de problème. En revanche, l'appel de `d.MiseAJour` ne fonctionnera
pas correctement, car la fonction `Entre` appelée dans `MiseAJour` est la
...
nécessaire de le faire que dans la classe de base (une méthode redéfinissant
une méthode virtuelle est automatiquement virtuelle). Celle-ci doit donc être
déclarée comme suit :
...
`DonneeBase`, soit celle de la classe `DonneeDetaillee` si `MiseAJour` est
appelée pour un objet de la classe `DonneeDetaillee`. Depuis C++11, il est
recommandé d'écrire `void Entre(void) override;` dans la classe dérivée : le
compilateur signale alors une erreur si la signature ne correspond à aucune
méthode virtuelle de la classe de base. Le spécificateur `final` interdit, lui,
toute redéfinition ultérieure.

En résumé, les méthodes virtuelles sont des méthodes qui sont appelées selon la
vraie classe de l'objet sur lequel elles sont appelées (cette résolution
dynamique n'a lieu qu'au travers d'un pointeur ou d'une référence). Les objets qui contiennent des méthodes
...
effet par héritage aux classes filles. Bien entendu, on doit avoir les droits
d'accès sur les membres de la classe de base que l'on utilise.

La deuxième règle indique qu'il est possible d'affecter un objet d'une
classe dérivée à un objet d'une classe mère. Les données qui ne servent pas à
l'initialisation sont perdues (on parle de *slicing*, ou troncature), puisque la classe mère ne possède pas les champs
...
classes de base. Il faut bien entendu en outre que la classe de base soit accessible (et non
ambiguë) à l'endroit de la conversion. Cette condition n'est pas vérifiée, par
exemple, en dehors de la classe dérivée pour une classe de base dont l'héritage
est `private`.
...
cette raison qu'un transtypage explicite est nécessaire pour ce type de
conversion (`static_cast`, ou `dynamic_cast` qui vérifie le type réel à
l'exécution lorsque la classe est polymorphe).
...
pf=(Fille *) &m;  // Cette fois, c'est légal, mais DANGEREUX !
          // En effet, m n'est pas un objet de la classe fille :
          // utiliser *pf comme tel a un comportement indéfini.
```
...
Fille *pf = new Fille;
pm = pf;
delete pm; // Comportement indéfini : en pratique, seul le
           // destructeur de la classe mère est appelé !
```
...
Une méthode virtuelle pure est une méthode qui est déclarée sans être
nécessairement définie dans une classe. Elle doit être redéfinie dans les
classes dérivées destinées à être instanciées. Une méthode virtuelle pure peut
néanmoins avoir une définition (c'est même obligatoire pour un destructeur
virtuel pur).

Une classe abstraite est une classe comportant au moins une méthode virtuelle
pure, déclarée ou héritée sans avoir été redéfinie.
...
référencer avec des pointeurs ou des références.
...
suivre sa déclaration de `= 0`. La fonction doit également être déclarée
virtuelle.
...
Par exemple, supposons que l'on veuille créer une structure de données pouvant contenir
...
le nom de la classe avec l'opérateur de résolution de portée. L'accès au membre
pointé se fait ensuite avec les opérateurs `.*` (sur un objet) et `->*` (sur un
pointeur d'objet) :
...
{
    int x, y;
    int somme() const { return x + y; }
};

int Point::*pm = &Point::x;                // Pointeur sur donnée membre.
int (Point::*pf)() const = &Point::somme;  // Pointeur sur méthode.
...
void exemple()
{
    ...
    Point *pp = &p;
    p.*pm = 3;                // Modifie p.x.
    int s = (pp->*pf)();      // Appelle p.somme() : s vaut 5.
}
```
