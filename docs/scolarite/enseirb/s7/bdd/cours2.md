...
séquentiels, indexage, adressage dispersé, chaînes de pointeurs, compression...)
dès lors qu'il est possible de relier ces structures à des tables au niveau
logique. Les tables ne représentent donc qu'une abstraction de
...
+ les données sont organisées sous forme de tables à deux dimensions, encore
  appelées relations, dont les lignes sont appelées $n$-uplets (*tuples* en
  anglais)
...
Une **occurrence**, ou $n$-uplet, ou tuple, est un élément de l'ensemble
figuré par une relation. Autrement dit, une occurrence est une ligne du tableau
qui représente la relation.

La **cardinalité** d'une relation est son nombre d'occurrences.

Une **super-clé** d'une relation est un ensemble d'attributs dont la valeur
permet d'identifier chaque tuple de manière unique.

Une **clé candidate** d'une relation est une super-clé minimale : aucun de ses
sous-ensembles stricts n'est une super-clé. Sa valeur est donc distincte pour
tous les tuples de la relation. La
...
La **clé primaire** d'une relation est une de ses clés candidates. Pour signaler
...
dont les valeurs doivent correspondre à celles d'une clé candidate (en général
la clé primaire) d'une autre relation (contrainte d'intégrité référentielle).
...
$n$-uplets des différentes relations du schéma relationnel.
...
3. Chaque type association dont aucune patte n'a pour cardinalité maximale 1
   ...
   identifiants des types entité qui interviennent dans le type association.
4. Un type association dont au moins une patte a une cardinalité maximale à 1
   ...
   comme attribut (et donc comme clé étrangère) l'identifiant de l'autre type
   entité.
...
Le schéma relationnel adéquat correspondant au modèle ci-dessus devient donc :
...
redondance dans les bases de données relationnelles afin d'éviter ou de
...
certaines de ses projections afin que la jointure de ces projections permette de retrouver
la relation initiale. En d'autres termes, le processus est réversible (c.-à-d. sans
...
5e forme normale est forcément en 4e forme normale, elle-même forcément en forme
...
relationnel sont toujours en forme normale, mais, sauf erreur, il ne
devrait pas y avoir de problème. Il en va tout autrement lorsque l'on ne connaît
pas bien, ou ne maîtrise pas bien, l'origine d'un modèle relationnel. Dans ce cas,
...
le modèle de conception (c.-à-d. on ne passe pas par un modèle entités-associations).

Nous abordons ici la normalisation en nous appuyant sur les notions de dépendance
fonctionnelle, dépendance multivaluée et dépendance de jointure. Il est important de prendre conscience que la dépendance fonctionnelle, la
dépendance multivaluée et la dépendance de jointure sont des notions
sémantiques. Elles trouvent leur origine dans les contraintes du monde réel.
...
être spécifiées dans la définition de la base de données afin que le SGBD puisse
...
de données est une manifestation observable des dépendances observées dans le
...
`Dépendance fonctionnelle :` Soit $R(A_1,A_2,...,A_n)$ un schéma de relation, et $X$ et $Y$ des sous-ensembles
de $\{A_1, A_2, \ldots, A_n\}$. On dit que $X$ détermine $Y$ ou que
...
\rightarrow Y$, si, connaissant une valeur de $X$, on ne peut lui associer
qu'une seule valeur de $Y$.
...
$X'$ inclus au sens strict dans $X$ (c.-à-d. $X' \subset X$) tel que $X'
\rightarrow A$.

Autrement dit, une dépendance fonctionnelle est élémentaire si la cible est un
attribut unique et si la source ne comporte pas d'attributs superflus. La
...
\rightarrow A$ est une dépendance fonctionnelle directe s'il n'existe aucun
ensemble d'attributs $B$ (avec $B \nrightarrow X$ et $A \notin B$) tel que l'on
puisse avoir $X \rightarrow B$ et $B \rightarrow A$.
...
et seulement si, tout attribut contient une valeur atomique (non multiple, non
composée). Par exemple, le pseudo schéma de relation `Personne(num-personne,
...
valeur pour chaque colonne (c.-à-d. attribut), ce qui est justement la définition
...
Dans la formulation usuelle (Codd), la condition porte sur chaque attribut
*non premier* (qui n'appartient à aucune clé candidate) : il doit dépendre
pleinement de chaque clé candidate, et non d'une partie seulement de l'une
d'elles.
...
Formulation équivalente usuelle : une relation est en 3FN si, pour toute
dépendance fonctionnelle non triviale $X \rightarrow A$, soit $X$ est une
super-clé, soit $A$ appartient à une clé candidate.
...
élémentaires sont celles dans lesquelles une clé candidate entière détermine un
attribut. De manière équivalente : pour toute dépendance fonctionnelle non
triviale $X \rightarrow Y$, $X$ est une super-clé.

Cette forme normale permet de combler certaines lacunes de la troisième forme
...
encore d'autres cas de redondance, mais qui ne sont pas expliqués par des
...
`Dépendance multivaluée (DM) :` Soit $R(A_1,A_2,\ldots,A_n)$ un schéma de relation
contenant $n$ propriétés, soit $X$ et $Y$ des sous-ensembles de
$\{A_1,A_2,\ldots,A_n\}$, $Z$ l'ensemble des propriétés restantes
($Z = R - (X \cup Y)$), et soit $X_i, Y_i$ et $Z_i$ des instances de ces
sous-ensembles (c.-à-d. une affectation de valeur à chacune des propriétés de ces
sous-ensembles). Il existe une dépendance multivaluée (DM) $X \twoheadrightarrow Y$
lorsque :
...
Nom-Employé $\twoheadrightarrow$ Type-intervention et Nom-Employé
$\twoheadrightarrow$ Marque.

#### Quatrième forme normale

`Quatrième forme normale :` Une relation est en quatrième forme normale (4FN)
si, et seulement si, elle est en forme normale de Boyce-Codd et si, pour toute
dépendance multivaluée non triviale $X \twoheadrightarrow Y$, $X$ est une
...
remplaçant par deux de ses projections. Il existe cependant des relations qui ne
...
Les dépendances de jointure font appel à des notions (projection et jointure)
qui seront définies plus loin.

`Dépendance de jointure (DJ) :` Soit $X_1, X_2,\ldots,X_n$ des sous-ensembles
d'un schéma de relation $R$. Il y a une dépendance de jointure, notée
$\ast\{X_1,X_2,\ldots,X_n\}$, dans la relation $R$ si :
...
`Cinquième forme normale (5FN) :` Une relation $R$ est en cinquième forme
normale (5FN) si, pour toute dépendance de jointure non triviale
$\ast\{X_1,X_2,\ldots,X_n\}$ dans $R$ chacun des $X_i$ contient une clé
...
clé candidate de la table. Il est donc superflu de décomposer de ce point de vue.
...
anomalie pouvant être supprimée en effectuant des projections (c.-à-d. des
décompositions).
...
Bien que l'objectif de la normalisation soit d'amener le concepteur à obtenir
des relations en forme normale finale (c.-à-d. en cinquième forme normale), cet
