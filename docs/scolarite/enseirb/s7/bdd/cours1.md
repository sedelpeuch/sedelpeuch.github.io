...
d'informations (c.-à-d. stocker des données et manipuler ces données). De plus, les
données peuvent être de natures diverses, les opérations peuvent être plus ou
moins compliquées.

### Objectifs

C'est dans ce contexte qu'interviennent les SGBD, dont les objectifs sont les
suivants :
...
Pour répondre à ces objectifs, les SGBD sont munis des
...
+ Contrôle de concurrence : propriétés ACID, transactions
  ...
  + Isolation (quelle que soit la manière dont les transactions concurrentes
      ...
      de la base soit le même qu'après une exécution séquentielle des différentes
      transactions)
...
+ Indépendance logique (une modification au niveau logique n'implique aucune
...
+ Analogues respectivement aux notions de type et de variable dans les
...
+ Ensemble d'outils permettant de définir les schémas et les instances ainsi que
...
Le passage de la 2 à la 3 peut être en partie automatisé.
...
pour lui (entier, chaîne de caractères, booléen...). Au niveau de l'entité,
chaque attribut possède une valeur compatible avec son domaine.

**Règle 5** : Un attribut ne peut en aucun cas être partagé par plusieurs types
entité ou types association.

**Règle 6** : Un attribut est une donnée élémentaire, ce qui exclut des données calculées ou
dérivées.
...
plusieurs types entité. Les associations de ce type association lient des entités
de ces types entité.
...
implicites.
Une association est souvent nommée occurrence ou instance de son type
association.
...
La `dimension`, ou l'arité, d'un type association est le nombre de types
entité contenus dans la collection.

Comme un type entité, un type association possède forcément un identifiant qu'il soit
explicite ou non.

**Règle 16** : La concaténation des identifiants des types entités liés à un type association
...
Exemple de cardinalité : une personne peut être l'auteur de 0 à $n$ livres,
...
considérons qu'elle est indéterminée et vaut $n$. En effet, même si nous
...
départ. De la même manière, on ne modélise pas de cardinalités minimales qui
...
Une occurrence de ce type association (c.-à-d. une association) associe généralement
une occurrence du type entité (c.-à-d. une entité) à une autre entité du même
...
type association met en relation $n$ types entité. Même s'il n'y a, en
principe, pas de limite sur l'arité d'un type association, dans la pratique on
va rarement au-delà de trois. Les associations de degré supérieur à deux sont
...
La présence des deux types entité Enseignant et Étudiant est symptomatique d'une
modélisation inachevée. À terme, ces deux types entité doivent être fusionnés en
...
cadre d'une occurrence du type association Correspondre, les valeurs des deux
attributs Adresse de facturation diffèrent ?

![Figure 2.17](./img/fig2_17.png)

Dans cette situation, les deux attributs Adresse doivent simplement être renommés
...
trouve. Il faut toutefois remarquer que le dernier cas décrit n'est pas
rédhibitoire et que les SGBD relationnels s'accommodent très bien de relations
...
**Règle 22** : Il faut remplacer un attribut multiple par un type association et un type entité supplémentaires.

![Figure 2.18](./img/fig2_18.png)

Remplacement des attributs multiples par un type association et un type entité et
...
portant sur la ville où habitent les employés si cette information est noyée
...
valeurs des attributs de base et celles des attributs dérivés. La figure
ci-dessus illustre le cas d'un attribut Montant total dans un type entité Commande
...
cardinalité maximale de l'une des pattes est 1 ne peut pas posséder d'attribut.
...
personne d'être à la fois Abonné et Écrivain.

![Figure 2.23](./img/fig2_23.png)
...
Un seul type association suffit pour remplacer les quatre types association Jouer
en tant que...
...
**Règle 28** : Lorsque les cardinalités d'un type association sont toutes 1,1, c'est
...
association doit généralement être supprimé et les types entité correspondants
...
la **règle 27** qu'il faut appliquer : on remplace le type entité Règlement par
...
Les formes normales sont différents stades de qualité qui
...
+ Une première fois, dans le cadre du modèle entités-associations, en
  privilégiant une approche plus intuitive qui n'introduit pas explicitement la
...
+ Puis une seconde fois, dans le cadre du modèle relationnel, en privilégiant
  une approche plus formelle s'appuyant sur la définition des dépendances
  fonctionnelles, multivaluées et de jointure. Nous irons alors jusqu'à la
...
l'objet d'une entité supplémentaire (comme l'attribut Occupants sur la figure
ci-dessus).
...
seulement si, il est en deuxième forme normale et si tout attribut n'appartenant
pas à la clé dépend directement de la clé, et non d'un autre attribut
n'appartenant pas à la clé (pas de dépendance transitive).
...
partie de la clé ne dépend d'un attribut ne faisant pas partie de la clé.
Plus formellement, tout attribut (ou ensemble d'attributs) dont dépend un autre
attribut doit être une clé candidate.

Intéressons-nous par exemple, sur la figure ci-dessus, à un type entité Diplômé
...
mais un même diplôme peut être délivré par plusieurs
...
Diplôme. Le type entité obtenu est en troisième forme normale, mais une
...
Il existe d'autres formes normales (quatrième et cinquième formes normales,
fondées sur les dépendances multivaluées et de jointure), présentées dans le
cadre du modèle relationnel.

## Élaboration d'un modèle entités-associations

### Étapes de conception d'un modèle entités-associations
...
prix de production (c.-à-d. le coût de revient industriel) soit le même que le prix de
...
production inférieur au prix de vente).
...
dépendent des attributs pouvant servir d'identifiant. Lorsqu'on ne parvient pas
...
Il est parfois difficile de faire un choix entre un type entité et un type
...
Étudiez également les cardinalités des types association retenus. Lorsque toutes
