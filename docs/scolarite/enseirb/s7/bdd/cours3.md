...
+ Les opérateurs binaires ensemblistes (Union, Intersection, Différence) : ces
  ...
  deux relations de même schéma (mêmes attributs, mêmes domaines).
...
utilise donc des notations courantes, mais pas forcément universelles.
...
satisfont l'expression logique $E$, on la note $\sigma_E(R)$. Il s'agit d'une
...
En d'autres termes, la sélection permet de choisir (c.-à-d. de sélectionner) des
lignes dans le tableau. Le résultat de la sélection est donc une nouvelle
relation qui a les mêmes attributs que $R$. Si $R$ est vide (c.-à-d. ne contient
aucune occurrence), la relation qui résulte de la sélection est vide.

Exemple : relation $\text{Personne}$.
...
Résultat de $\sigma_{\text{Numéro} \geq 5}(\text{Personne})$ :
...
$A_1, \ldots, A_n$ d'une relation et à éliminer les $n$-uplets en double
apparaissant dans la nouvelle relation, on la note $\pi_{A_1,\ldots,A_n}(R)$.
...
tableau. Si $R$ est vide, la relation qui résulte de la projection est vide,
mais pas forcément équivalente (elle contient généralement moins d'attributs).

Résultat de $\pi_{\text{Nom}}(\text{Personne})$ (le doublon Germain est
éliminé) :
...
des $n$-uplets appartenant à l'une ou l'autre des deux relations $R_1$ et $R_2$,
sans doublon, on la note $R_1 \cup R_2$. Il s'agit d'une opération binaire
...
est une nouvelle relation qui a les mêmes attributs que $R_1$ et $R_2$. Si
$R_1$ et $R_2$ sont vides, la relation qui résulte de l'union est vide. Si $R_1$
(respectivement $R_2$) est vide, la relation qui résulte de l'union est
identique à $R_2$ (respectivement $R_1$).
...
dont les $n$-uplets sont constitués de ceux appartenant aux deux relations, on
la note $R_1 \cap R_2$. Il s'agit d'une opération binaire ensembliste
commutative dont la signature est :
...
vides, la relation qui résulte de l'intersection est vide. L'intersection n'est
pas un opérateur primitif : $R_1 \cap R_2 = R_1 - (R_1 - R_2)$.
...
exclusivement toutes les possibilités de combinaison des occurrences des
relations $R_1$ et $R_2$, on la note $R_1 \times R_2$.
...
## Jointure, thêta-jointure, équijointure, jointure naturelle
...
possibilités de combinaison des occurrences des relations $R_1$ et $R_2$ qui
...
`Thêta-jointure :` Une thêta-jointure est une jointure dans laquelle l'expression
logique $E$ est une simple comparaison $A_1 \,\theta\, A_2$ (avec
$\theta \in \{=, \neq, <, \leq, >, \geq\}$) entre un attribut $A_1$ de la
relation $R_1$ et un attribut $A_2$ de la relation $R_2$. La
thêta-jointure est notée $R_1 \bowtie_{A_1 \theta A_2} R_2$.
...
est notée $R_1 \bowtie_{A_1 = A_2} R_2$.
...
d'attributs. La jointure naturelle est notée $R_1 \bowtie R_2$. On peut
préciser explicitement les attributs communs à $R_1$ et $R_2$ sur lesquels
porte la jointure : $R_1 \bowtie_{A_1, \ldots, A_n} R_2$.

Généralement, $R_1$ et $R_2$ n'ont qu'un attribut en commun. Dans ce cas, une
jointure naturelle est équivalente à une équijointure dans laquelle l'attribut
de $R_1$ et celui de $R_2$ sont justement les deux attributs qui portent le même
nom (à la fusion des deux colonnes près).

Lorsque l'on désire effectuer une jointure naturelle entre $R_1$ et $R_2$
sur un attribut $A_1$ commun à $R_1$ et $R_2$, il vaut mieux écrire
$R_1 \bowtie_{A_1} R_2$ que $R_1 \bowtie R_2$. En effet, si $R_1$ et $R_2$
possèdent deux attributs portant un nom commun, $A_1$ et $A_2$,
$R_1 \bowtie_{A_1} R_2$ est bien une jointure naturelle sur l'attribut $A_1$,
mais $R_1 \bowtie R_2$ est une jointure naturelle sur le couple d'attributs
$(A_1, A_2)$, ce qui produit un résultat très différent.
...
d'occurrences de la relation $R_1$ qui sont associées à toutes les occurrences
de la relation $R_2$ ; on la note $R_1 \div R_2$. Il s'agit d'une opération
...
Autrement dit, la division de $R_1$ par $R_2$ génère une relation qui
regroupe tous les $n$-uplets qui, concaténés à chacun des $n$-uplets de
$R_2$, donnent toujours un $n$-uplet de $R_1$. Tous les attributs de $R_2$
doivent être présents dans $R_1$ et $R_1$ doit posséder au moins un attribut de
plus que $R_2$ (inclusion stricte). Le résultat de la division est une nouvelle
relation qui a tous les attributs de $R_1$ sans aucun de ceux de $R_2$. Si $R_1$
est vide, la relation qui résulte de la division est vide. On suppose en général
$R_2$ non vide : d'après la définition, diviser par une relation vide
renverrait toutes les valeurs de $R_1$ projetées sur ses attributs propres.

La division n'est pas un opérateur primitif. En notant $X$ l'ensemble des
attributs de $R_1$ qui n'appartiennent pas à $R_2$ :

$$
R_1 \div R_2 = \pi_X(R_1) - \pi_X\big((\pi_X(R_1) \times R_2) - R_1\big)
$$
