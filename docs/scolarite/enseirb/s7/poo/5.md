...
description: "Substitution d'objets en Java : relation type / sous-type, lien est-un, construction interface, liaison statique et dynamique, polymorphisme."
---
...
+ un emplacement dans le tableau `mesPortes[numero]` va pouvoir référencer aussi
  bien une instance de la classe `PorteCharniere` qu'une instance de la classe
  `PorteCoulissante` ;
...
Pour rendre possible la substitution d'objets, il faut :

+ assurer la validité de l'envoi de message sur les instances de classes
  différentes référencées par la même variable ;
...
[public] interface NomInterface {
    ...
    [[public] type methode([liste paramètres]);]
}
```
...
+ Une interface contient les prototypes de méthodes d'instance abstraites
  ...
  réalisation). Les modificateurs `public` et `abstract` sont implicites.
+ Comme une classe, une interface doit être déclarée publique pour être
  accessible en dehors de son paquetage.
+ Il est possible de définir des variables de classe constantes publiques
  (`public static final` implicites). La valeur d'une telle constante est fixée
  à la définition de la variable de classe.
+ Jusqu'à Java 7, aucune méthode de classe n'était autorisée. Depuis Java 8, une
  interface peut contenir des méthodes `static` et des méthodes d'instance
  `default` avec du code (et depuis Java 9 des méthodes `private`), voir
  [Hiérarchie de classes](./7.md).

Une interface permet de définir une abstraction qui est mise en œuvre par des
classes concrètes. Voici la représentation de l'abstraction "porte" par une
interface :
...
Pour substituer les instances des classes `PorteCharniere` et
...
d'instance publiques. Cet héritage peut être assimilé à l'héritage d'un type
abstrait.
...
En plus de ses définitions propres (attributs, constructeurs, méthodes), la
classe va inclure la déclaration des prototypes des méthodes de toutes les
interfaces déclarées après la clause `implements`. Les méthodes "héritées" sont
abstraites. Pour obtenir une classe concrète à partir d'un héritage
d'interfaces, il est nécessaire de fournir le code pour chaque méthode déclarée
dans les interfaces en respectant le prototype de ces méthodes. Sinon, la classe
doit être déclarée abstraite (`abstract`), faute de quoi le compilateur signale
une erreur.
...
**Liaison statique** : la liaison entre l'envoi du message (nom + paramètres)
et l'adresse de la méthode s'effectue à la compilation. Le compilateur utilise
...
l'adresse de la méthode à l'exécution.

En Java, la liaison est dynamique pour les méthodes d'instance ; les méthodes
de classe (`static`) et les méthodes `private` sont liées statiquement. Le choix
entre plusieurs méthodes surchargées se fait, lui, à la compilation, d'après le
type déclaré des arguments.

**Définition** : le polymorphisme est la capacité d'un élément de texte logiciel
à désigner à l'exécution des formes différentes.

On parle de polymorphisme lorsqu'une variable (paramètre) peut référencer des
instances de plusieurs classes. Le polymorphisme nécessite la relation de
type / sous-type et la liaison dynamique. Dans un langage objet typé, la
substitution d'objets est rendue opérationnelle grâce au polymorphisme.
