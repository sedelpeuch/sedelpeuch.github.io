...
class ClientChannel(Channel):
    ...
    def Network_demandePartie(
        self, data
    ):  # Permet de demander une partie à quelqu'un et gère les différentes solutions
        #
        joueurA = self
        joueurB = [
            p for p in self._server.players if p.nickname == data["pseudoJoueur"]
        ][0]
        #
        if joueurB.nickname == None:
            self.Send(
                {
                    "action": "refusDemande",
                    "raison": "ERREUR : Un problème est survenu.",
                }
            )
            return None
        if self._server.players[joueurB] != 0:
            self.Send(
                {
                    "action": "refusDemande",
                    "raison": "{0} est déjà en partie!".format(data["pseudoJoueur"]),
                }
            )
            return None
        if data["pseudoJoueur"] == "0":
            self.Send(
                {
                    "action": "refusDemande",
                    "raison": "ERREUR : Le joueur n'est pas connecté.",
                }
            )
            return None
        #       - OK -
        joueurB.Send(
            {
                "action": "requetePartie",
                "pseudo": joueurA.nickname,
                "pointsA": self._server.score[joueurA],
                "pointsB": self._server.score[joueurB],
            }
        )
    ...
    def Network_partieRefus(self, data):
        ...
        joueurB.Send(
            {
                "action": "refusDemande",
                "raison": "{0} a refusé votre demande de partie!".format(self.nickname),
            }
        )
    ...
    def Network_partieAccepte(self, data):
        ...
        if self._server.players[joueurB] != 0:
            self.Send(
                {
                    "action": "refusDemande",
                    "raison": "{0} est déjà en partie!".format(data["pseudoB"]),
                }
            )
            return None
        if self._server.players[self] != 0:
            self.Send(
                {"action": "refusDemande", "raison": "Vous êtes déjà en partie !"}
            )
            return None
        #
        if randint(0, 1) == 0:
    ...
    def Network_coup(self, data):  # Permet de réaliser un coup
        [
            p.Send({"action": "coup", "coup": data["coup"]})
...
class MyServer(Server):
    ...
    def actualiserJoueur(self):
        liste = [
            [p.nickname, self.score[p], self.players[p]]
            for p in self.players
            if p.nickname != "0"
        ]
        [
            p.Send({"action": "afficherJoueur", "liste": liste})
            for p in self.players
            if p.nickname != "0"
        ]
...
class Partie:
    ...
    def fin(
        self, gagnantp
    ):  # Gère la fin de la partie, permet de dire qui est le perdant et qui est le gagnant
